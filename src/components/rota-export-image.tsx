...
export const RotaExportImage = React.forwardRef<HTMLDivElement, RotaExportImageProps>(
    ({ activeGeneration }, ref) => {
        const { teamMembers, showExportFooter } = useRotaStore();
        const startDate = parseISO(activeGeneration.startDate);
        const endDate = parseISO(activeGeneration.endDate);
        
        const title = `Period: ${format(startDate, 'd MMM yyyy')} - ${format(endDate, 'd MMM yyyy')}`;
        ...
        return (
            <div ref={ref} className="px-8 py-4 bg-white text-black font-body w-[1200px]">
                ...
                {showExportFooter && <p className="text-xs text-gray-500 mt-4 text-center">Generated by DeltaPro on {format(new Date(), 'PPpp')}</p>}
            </div>
        );
    }
);
